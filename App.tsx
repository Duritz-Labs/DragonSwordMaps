...
import { Compass, ChevronLeft, ChevronRight, Trash2, X, AlertCircle, Lock, Layers, Shield, User as UserIcon, Download, Upload, Copy, Check, Search, FileText, Loader2 } from 'lucide-react';
...
/**
 * 보안 및 환경 호환성을 위해 Base64 방식으로 비교합니다.
 * 10051 -> "MTAwNTE="
 */
...
/**
 * 현재 시점 기준으로 가장 최근의 '월요일 오전 9시' 타임스탬프를 계산합니다.
 */
const getLastMonday9AM = (now: Date) => {
  const d = new Date(now);
  d.setHours(9, 0, 0, 0);
  const day = d.getDay(); // 일:0, 월:1, ...
  const diff = (day + 6) % 7; // 지난 월요일까지의 거리
  d.setDate(d.getDate() - diff);
  
  // 만약 계산된 시간이 현재보다 미래라면 (오늘이 월요일인데 9시 이전인 경우 등), 일주일 전으로 돌림
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkWeeklyReset = () => {
      ...
      // 마지막 초기화 시점이 최근 월요일 9시보다 이전이라면 초기화 수행
      if (lastResetTime < lastMonday9AM) {
        setFadedPinKeys(prev => {
          // '지역(퀘)'와 '돌발(토)' 타입만 필터링하여 제거
  ...
  const handleLocationClick = useCallback((loc: Location) => {
    ...
    if (transformComponentRef.current) {
      ...
      const targetY = (loc.y / 100) * CANVAS_HEIGHT;
      const currentSidebarWidth = isSidebarOpen ? SIDEBAR_WIDTH : 0;
      const visibleWidth = window.innerWidth - currentSidebarWidth;
      const x = -(targetX * zoomLevel - (visibleWidth / 2 + currentSidebarWidth));
      const y = -(targetY * zoomLevel - window.innerHeight / 2);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    pressStartPos.current = { x: startX, y: startY };
  ...
  const updatePin = () => {
    ...
    setCustomPins(prev => prev.map(p => 
      p.id === editingPin.id 
        ? { ...p, type: activePinType, comment: pinComment }
        : p
    ));
    setEditingPin(null);
    setActivePinType(null);
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (newPins.length > 0) {
          ...
          setFadedPinKeys(prev => {
            const combined = Array.from(new Set([...prev, ...newFadedKeys]));
            return combined;
          });
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-[#020617] select-none text-slate-200">
      ...
      {mapLoading && (
        <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col items-center justify-center transition-opacity duration-1000">
          ...
          <div className="mt-8 space-y-2 text-center">
            <h2 className="font-fantasy text-xl text-amber-100 tracking-[0.3em] animate-pulse uppercase">지도 데이터를 불러오는 중...</h2>
            <p className="text-[10px] text-amber-500/60 font-bold uppercase tracking-widest">대륙의 전경을 캔버스에 그리는 중입니다</p>
          </div>
        </div>
      )}
      ...
      <aside className={`fixed inset-y-0 left-0 w-80 bg-slate-950/70 backdrop-blur-xl border-r border-amber-500/20 flex flex-col shadow-[20px_0_50px_rgba(0,0,0,0.5)] z-40 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="h-full flex flex-col p-6 overflow-hidden">
          ...
          <div className="flex-none mb-6">
            <p className="text-[11px] text-amber-500/80 font-bold uppercase tracking-[0.2em] mb-3 border-b border-amber-500/10 pb-2 flex items-center gap-2"><Layers size={12} /> 맵핀 필터</p>
            <div className="grid grid-cols-4 gap-2 w-full">
              <button onClick={() => setSelectedFilters([])} className={`relative flex flex-col items-center justify-center py-2 rounded-lg text-[13px] font-bold border transition-all text-center overflow-hidden ${selectedFilters.length === 0 ? 'bg-amber-600/30 border-amber-400 text-amber-100 shadow-[0_0_10px_rgba(245,158,11,0.2)]' : 'bg-slate-800/40 border-white/5 text-slate-500 hover:text-slate-300'}`}>
              ...
              {PIN_TYPES.map(pt => (
                <button 
                  ...
                  className={`relative flex flex-col items-center justify-center aspect-square rounded-lg border transition-all overflow-hidden ${selectedFilters.includes(pt.type) ? 'border-amber-400 shadow-lg scale-[1.02]' : 'border-white/5 opacity-70 hover:opacity-100'}`}
                >
                  {/* Icon as full background */}
                  <img src={PIN_IMAGES[pt.type]} alt={pt.label} className="absolute inset-0 w-full h-full object-cover" />
                  
                  {/* Label overlay (bottom 1/3) */}
                  <div className="absolute bottom-0 left-0 w-full h-1/3 bg-black/60 flex items-center justify-center px-0.5">
                    <span className="text-[12px] font-bold text-white truncate w-full text-center tracking-tighter">{pt.label}</span>
                  </div>
          ...
          <div className="flex-none mt-4 mb-6">
            <p className="text-[11px] text-amber-500/80 font-bold uppercase tracking-[0.2em] mb-3 border-b border-amber-500/10 pb-2 flex items-center gap-2">
              <Layers size={12} /> 데이터 관리
            </p>
            <div className={`grid grid-cols-1 gap-2`}>
              <button onClick={() => fileInputRef.current?.click()} className="flex flex-row items-center justify-center py-2.5 px-4 bg-slate-800/50 border border-white/10 rounded-xl hover:bg-slate-800 transition-all gap-2 group">
                <Upload size={14} className="text-slate-400 group-hover:text-white" />
                <span className="text-xs font-bold text-slate-400 group-hover:text-white">맵핀정보 불러오기</span>
              </button>
              <button onClick={exportCSV} className="flex flex-row items-center justify-center py-2.5 px-4 bg-amber-600/10 border border-amber-500/30 rounded-xl hover:bg-amber-500/20 transition-all gap-2 group">
                <FileText size={14} className="text-amber-500 group-hover:scale-110" />
                <span className="text-xs font-bold text-amber-200">맵핀정보 내보내기</span>
              </button>
            </div>
          </div>
      ...
      <main className={`h-full w-full relative overflow-hidden transition-opacity duration-1000 ${mapLoading ? 'opacity-0' : 'opacity-100'}`}>
        <TransformWrapper ref={transformComponentRef} initialScale={0.75} minScale={0.4} maxScale={4} limitToBounds={true} disabled={!!pendingPin || !!deletingPinId || !!editingPin || !!infoPin}>
          {() => (
                    ...
                    <div className="absolute inset-0 w-full h-full pointer-events-none">
                      {filteredPins.map((pin) => (
                        <CustomMarker 
                          ...
                          x={pin.x} 
                          y={pin.y} 
                          isAdmin={entryMode === 'ADMIN'} 
                          // 관리자 모드에서는 탐색완료 여부와 상관없이 선명하게 표시
                          isFaded={entryMode === 'USER' && fadedPinKeys.includes(getFadedKey(pin))}
                          onDelete={() => requestDeletePin(pin.id)} 
                          onClick={() => entryMode === 'ADMIN' ? startEditPin(pin) : setInfoPin(pin)}
                        />
                      ))}
                    </div>
                    {pendingPin && (
      ...
      {infoPin && (
        <div 
          className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300"
        ...
        >
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-7 shadow-2xl w-80 space-y-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-2">
               ...
               <div className="bg-slate-950/50 rounded-xl p-4 border border-white/5 text-sm text-slate-200 leading-relaxed min-h-[80px]">
                 {infoPin.comment || "상세 설명이 없습니다."}
               </div>
            </div>
            <button 
              onClick={() => toggleFindPin(infoPin)}
              className={`w-full py-4 flex items-center justify-center gap-2 font-bold rounded-2xl transition-all active:scale-95 shadow-lg ${fadedPinKeys.includes(getFadedKey(infoPin)) ? 'bg-slate-800 text-slate-400 hover:bg-slate-700' : 'bg-amber-600 text-white hover:bg-amber-500'}`}
            >
              <Search size={18} />
      ...
      {(pendingPin || editingPin) && (
        <div 
          className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 backdrop-blur-sm animate-in fade-in duration-300"
          onClick={() => { setPendingPin(null); setEditingPin(null); }}
        >
          <div className="bg-slate-900 border border-amber-500/30 rounded-2xl p-6 shadow-2xl w-80 space-y-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <span className="text-sm font-bold text-amber-500 uppercase tracking-widest flex items-center gap-2">
                <Compass size={18} /> {editingPin ? "맵핀 수정" : "맵핀 추가"}
              </span>
            ...
            <div className="grid grid-cols-4 gap-2">
              {PIN_TYPES.map(pt => (
                <button 
                  key={pt.type} 
                  onClick={() => setActivePinType(pt.type)} 
                  className={`relative aspect-square rounded-xl flex items-center justify-center border transition-all overflow-hidden ${activePinType === pt.type ? `border-amber-400 ring-2 ring-amber-400 scale-105 shadow-[0_0_15px_rgba(245,158,11,0.3)]` : `bg-slate-800/40 border-white/5 hover:border-white/20`}`}
                >
                  <img src={PIN_IMAGES[pt.type]} alt={pt.label} className="absolute inset-0 w-full h-full object-cover" />
                  <div className="absolute bottom-0 left-0 w-full h-1/3 bg-black/60 flex items-center justify-center px-0.5">
                    <span className="text-[12px] font-bold text-white truncate w-full text-center tracking-tighter">{pt.label}</span>
            ...
            <div className="space-y-4">
              ...
              <button onClick={editingPin ? updatePin : createPin} disabled={!activePinType} className="w-full py-3 bg-amber-600 hover:bg-amber-500 disabled:bg-slate-800 disabled:text-slate-600 text-white text-sm font-bold rounded-xl transition-all shadow-lg active:scale-95">
                {editingPin ? "정보 수정하기" : "핀 설치하기"}
              </button>
            </div>
          </div>
        </div>
